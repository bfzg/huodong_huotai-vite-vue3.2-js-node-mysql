<template>
  <div class='individual'>
    ...
    <el-card>
      ...
      <!-- 分页 -->
      <el-pagination @current-change="handleSizeChange" :current-page="requestParams.pageNum" :page-size="requestParams.pageSize" background layout="prev, pager, next" :total="total" />
    </el-card>
    ...
    <el-dialog v-model="dialogVisible" title="编辑活动" width="50%" :before-close="handleClose" :show-close="false">
      <el-form :rules="rules" ref="formTable" :model="newEventsValue" label-width="100px"  label-position="left">
        <el-form-item label="用户名">
          <el-input v-model="newEventsValue.uname" prop="name"/>
        </el-form-item>
        <el-form-item label="邮箱">
          <el-input v-model="newEventsValue.uemail" prop="name" />
        </el-form-item>
        <el-form-item label="开始时间" prop="name">
          <el-date-picker
          v-model="newEventsValue.starttime"
          type="date"
      ...
      <el-form-item label="结束时间" prop="name">
        <el-date-picker
          v-model="newEventsValue.shuttime"
          type="date"
          placeholder="Pick a date"
          style="width: 100%"
        />
...
//TODO这里经行表单验证操作 必须要验证成功才能修改
//表单验证
const rules = reactive({
  name: [
    { required: true, message: '请输入内容', trigger: 'blur' },
    { min: 1, max: 20, message: '长度在1 to 20 之间', trigger: 'blur' },
  ],
})
